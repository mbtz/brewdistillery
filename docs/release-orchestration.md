...
5. Discover the GitHub Release when using `release-asset` strategy.
6. Select the release asset(s) and compute SHA256 values.
...
Notes:
- In `--dry-run` mode, the command skips downloads, file writes, commits, and
  pushes, but it still queries the GitHub Release API to resolve inputs.
- When `tap.path` is missing but `tap.remote` (or `--tap-remote`) is present,
  the tap repo is cloned into a temp dir with prefix `brewdistillery-tap-`.
...
1. `--tap-path`
2. `tap.path`
3. If `tap.formula_path` is absolute, use it directly.
4. If a tap remote URL is available, auto-clone into a temp dir.
5. Otherwise, continue without a tap root and fail during required-field
   validation.
...
Tap path missing but tap remote is configured | Auto-clone and continue | 0
...
Example dry-run output when a tap repo is auto-cloned:

```text
release: cloned tap repo from git@github.com:acme/homebrew-tool.git to /tmp/brewdistillery-tap-XXXX/tap
dry-run: would download https://github.com/acme/tool/releases/download/1.2.3/tool-1.2.3-darwin-arm64.tar.gz
Repo: tap (/tmp/brewdistillery-tap-XXXX/tap)
